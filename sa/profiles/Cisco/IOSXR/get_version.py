...
class Script(BaseScript):
    ...
    rx_snmp_ver = re.compile(
        r"Cisco IOS XR Software \(Cisco (?P<platform>\S+(?:\s+XRv)?)\s+\w+\).+\s+"
    ...
    def execute(self):
        if self.has_snmp():
            try:
                ...
                platform = self.snmp.get("1.3.6.1.2.1.47.1.1.1.1.13.24555730", cached=True)  #IOSXR platform
                if not match:
                    match = self.rx_snmp_ver2.search(v)
                return {
                    "vendor": "Cisco",
                    "platform": platform if platform else match.group("platform"),
